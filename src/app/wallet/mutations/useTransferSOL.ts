    ...
    try {
      ...
      if (USE_MOCK) {
        ...
        const fromBalance = parseFloat(
          localStorage.getItem(`wallet_balance_${variables.from.toString()}`) ||
            '0',
        );
        const toBalance = parseFloat(
          localStorage.getItem(`wallet_balance_${variables.to}`) || '0',
        );
        localStorage.setItem(
          `wallet_balance_${variables.from.toString()}`,
          (fromBalance - variables.amount).toString(),
        );
        localStorage.setItem(
          `wallet_balance_${variables.to}`,
